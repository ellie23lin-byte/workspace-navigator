...
const DOCUMENT_ID = 'user_tool_order_v2'; // 使用新 ID 避免格式衝突
...
const App = () => {
    ...
    // 統一儲存函式
    const saveAllToFirebase = (ai, wf, md) => {
    ...
    const handleAdd = (type) => {
        ...
        const newTool = {
            ...
            desc: "自定義工具",
            icon: "external-link",
            color: "bg-white text-stone-600 border-stone-200", // 預設白底
            bgColor: "bg-stone-50 border-stone-100 text-stone-600" // 針對側邊欄格式
        };
    ...
    const handleDelete = (type, id, e) => {
        ...
        if (type === 'ai') {
            const next = aiTools.filter(t => t.id !== id);
            setAiTools(next);
            saveAllToFirebase(next, workflowTools, mediaTools);
        } else if (type === 'wf') {
            const next = workflowTools.filter(t => t.id !== id);
            setWorkflowTools(next);
            saveAllToFirebase(aiTools, next, mediaTools);
        } else {
            const next = mediaTools.filter(t => t.id !== id);
            setMediaTools(next);
            saveAllToFirebase(aiTools, workflowTools, next);
        }
    };
            ...
            if (ref.current) {
                Sortable.create(ref.current, {
                    ...
                    onEnd: (evt) => {
                        ...
                        const reordered = newIds.map(id => list.find(t => t.id === id)).filter(Boolean);
                        setter(reordered);
                        
                        // 更新資料庫
                        if (type === 'ai') saveAllToFirebase(reordered, workflowTools, mediaTools);
                        if (type === 'wf') saveAllToFirebase(aiTools, reordered, mediaTools);
                        if (type === 'md') saveAllToFirebase(aiTools, workflowTools, reordered);
                    }
                });
            }
        };
    ...
    if (loading) return <div className="min-h-screen flex items-center justify-center font-mono text-stone-400 uppercase tracking-widest">Loading Workspace...</div>;
        ...
        <div className="min-h-screen">
            <header className="sticky top-0 z-50 bg-[#FDFCF5]/80 backdrop-blur-md border-b border-stone-200 px-6 py-4 flex justify-between items-center">
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        <h1 className="text-lg font-bold text-stone-700 leading-none">Studio</h1>
                        <p className="text-[9px] text-stone-400 font-bold uppercase tracking-[0.2em] mt-1">Management Mode</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                    ...
                    <section className="lg:col-span-8">
                        <div className="flex items-center justify-between mb-8">
                            ...
                            <button onClick={() => handleAdd('ai')} className="p-2 bg-stone-100 hover:bg-stone-200 rounded-full transition-colors text-stone-500">
                        ...
                        <div ref={aiRef} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {aiTools.map((tool) => (
                                <div key={tool.id} data-id={tool.id} className="group relative p-5 bg-white border border-stone-200 rounded-[2rem] hover:shadow-xl transition-all cursor-grab active:cursor-grabbing">
                                    <button onClick={(e) => handleDelete('ai', tool.id, e)} className="absolute -top-2 -right-2 w-6 h-6 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center shadow-lg">
                    ...
                    <aside className="lg:col-span-4 space-y-10">
                        ...
                        <section>
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <button onClick={() => handleAdd('wf')} className="text-stone-400 hover:text-stone-600"><i data-lucide="plus-circle" className="w-5 h-5"></i></button>
                            </div>
                            <div ref={workflowRef} className="grid grid-cols-2 gap-3">
                                {workflowTools.map((tool) => (
                                    <div key={tool.id} data-id={tool.id} className="group relative">
                                        ...
                                        <a href={tool.url} target="_blank" className={`flex flex-col items-center justify-center p-5 ${tool.bgColor || 'bg-stone-50 text-stone-600 border-stone-100'} rounded-[2rem] border transition-all h-full text-center`}>
                        ...
                        <section>
                            <div className="flex items-center justify-between mb-6">
                                ...
                                <button onClick={() => handleAdd('md')} className="text-stone-400 hover:text-stone-600"><i data-lucide="plus-circle" className="w-5 h-5"></i></button>
                            </div>
